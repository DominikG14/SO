...
void check_config(){
    ...
    if(COMPLEX_CLOSE_TIME < COMPLEX_OPEN_TIME){

    }

    if(COMPLEX_CLOSE_TIME < time_HHMM(0, 0) || COMPLEX_CLOSE_TIME > time_HHMM(23, 59)){

    }

    if(COMPLEX_CLOSE_TIME < TIME_START){

    }

    if(CLIENT_SPAWN_PERC < 0 || CLIENT_SPAWN_PERC > 100){

    }

    if(CLIENT_HAS_CHILD_PERC < 0 || CLIENT_HAS_CHILD_PERC > 100){
